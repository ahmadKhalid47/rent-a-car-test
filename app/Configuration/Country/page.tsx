...
export default function Vehicles() {
  ...
  async function save(action: string) {
    if (country.trim() === "") {
      ...
      vehiclesData.find(
        (item) => item.country?.toLowerCase() === country.trim().toLowerCase()
      )
    ) {
  ...
  function filterVehicles() {
    ...
    const filtered = vehiclesData.filter((vehicle) => {
      const { country } = vehicle;

      return country?.toLowerCase().includes(lowercasedQuery);
    });
      ...
      >
        ...
        <div className="w-full h-fit">
          <div className="w-full h-fit mt-4 flex justify-between items-center">
            ...
            <ImportExportButtons data={vehiclesData} model={"Country"} />
          </div>
