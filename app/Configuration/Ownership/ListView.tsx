...
await axios.delete(`/api/deleteSingleItem/${_id}`, {
  ...
  async function deleteManyItem() {
    try {
      ...
      dispatch(setVehicleDataReloader(global.vehicleDataReloader + 1));
      dispatch(setAlert("Selective Ownerships Deleted Successfully"));
    } catch (err) {
      console.log(err);
    } finally {
      setDeleteLoading(false);
  ...
  async function editItem(_id: any) {
    ...
    try {
      ...
      dispatch(setVehicleDataReloader(global.vehicleDataReloader + 1));
      dispatch(setAlert("Selective Ownership Updated Successfully"));
    } catch (err) {
      console.log(err);
    } finally {
      ...
      <div className="w-full h-fit overflow-auto rounded-[10px] border-2 border-grey mt-2">
        <div className="w-[900px] 1200:w-full h-fit flex flex-col justify-start items-start dark:bg-dark2 bg-light-grey-2 overflow-hidden mt-0 leading-[17px]">
          ...
          {paginatedData.length < 1 ? (
            <span className="p-3">No Ownerships found.</span>
          ) : (
