...
await axios.delete(`/api/deleteSingleItem/${_id}`, {
  ...
  async function deleteManyItem() {
    try {
      ...
      dispatch(setVehicleDataReloader(global.vehicleDataReloader + 1));
      dispatch(setAlert("Selective Colors Deleted Successfully"));
    } catch (err) {
      console.log(err);
    } finally {
      setDeleteLoading(false);
              ...
              <div key={index} className="w-full">
                ...
                {editPopup ? (
                  <div className="items-center w-full h-full dark:bg-blackOpacity bg-[rgba(255,255,255,0.9) rounded-[10px] absolute top-0 left-0 flex justify-center item-center sm:items-center z-[10]">
                    <div className="w-[90%] sm:w-[800px] h-[430px] border-[1px] border-grey rounded-[10px] mt-0 flex flex-col justify-between items-end gap-x-[4%] gap-y-5 dark:bg-dark1 bg-white z-[15]  py-3 xs:py-5 md:py-14 px-1 xs:px-3 md:px-10 fixed modal-position animate-zoom">
                      <div
